...
import numpy as np
from deepclustering2.utils import simplex, one_hot, class2one_hot, probs2one_hot, to_float
from torch import Tensor

from .metric import Metric
from .utils import mod_hausdorff_distance, hausdorff_distance, average_surface_distance


class SurfaceMeter(Metric):
    meter_choices = {"mod_hausdorff": mod_hausdorff_distance, "hausdorff": hausdorff_distance,
                     "average_surface": average_surface_distance}
    abbr = {"mod_hausdorff": "MHD", "hausdorff": "HD", "average_surface": "ASD"}

    def __init__(self, C=4, report_classes=None, meter_name: str = "hausdorff") -> None:
        ...
        if report_classes is not None:
            self._report_axis = report_classes
        assert meter_name in self.meter_choices.keys()
        self._surface_name = meter_name
        self._abbr = self.abbr[meter_name]
        self._surface_function = self.meter_choices[meter_name]
        self.reset()
    ...
    def summary(self) -> dict:
        ...
        return {f"{self._abbr}{i}": to_float(means[num]) for num, i in enumerate(self._report_axis)}
    ...
    def _evalue(self, pred: Tensor, target: Tensor, voxel_spacing):
        ...
        for b, (one_batch_img, one_batch_gt) in enumerate(zip(pred, target)):
            for c, (one_slice_img, one_slice_gt) in enumerate(
                ...
                mhd = self._surface_function(one_slice_img, one_slice_gt, voxel_spacing=voxel_spacing)
                result[b, c] = mhd
        return result
    ...
    def _convert2onehot(self, pred: Tensor, target: Tensor):
        ...
        return class2one_hot(pred, self._C).long(), class2one_hot(target, self._C).long()
