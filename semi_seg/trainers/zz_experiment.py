...
from semi_seg.epochers.experiment import ExperimentalEpocher as TrainEpocher


class SemiTrainer2(Trainer):
    ...
    def __init__(self, *, model: nn.Module, labeled_loader: T_loader, unlabeled_loader: T_loader,
                 ...
                 max_epoch: int = 100, num_batches: int = 100, device: str = "cpu", disable_bn=False, two_stage=False,
                 configuration=None):
        ...
        self._train_with_two_stage = two_stage
        self._hook_list = nn.ModuleList()
    ...
    def _init_scheduler(self, optimizer):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            self._optimizer, T_max=self._config["Trainer"]["max_epoch"] - self._config["Scheduler"]["warmup_max"],
            eta_min=1e-7)
    ...
    def _init_optimizer(self):
        ...
        self._optimizer = optim.__dict__[optim_dict["name"]](
            params=filter(lambda p: p.requires_grad, chain(self._model.parameters(), self._hook_list.parameters())),
    ...
    def _run_init(self):
        ...
        epocher = self.epocher_class(
            ...
            cur_epoch=self._cur_epoch, device=self._device, two_stage=self._train_with_two_stage,
            disable_bn_for_regularize=self._disable_bn
        )
        epocher.set_trainer(self)
        for h in self._hook_list:
            epocher.add_hook(h())
        return epocher
    ...
    def start_training(self, *args, **kwargs):
        ...
        return super(SemiTrainer2, self).start_training(*args, **kwargs)
...
class FineTuneTrainer2(SemiTrainer2):
