...
python run_script_0228.py ${comm_cmd} --save_dir ${save_dir} -n acdc -b ${batch_size} -s ${rand_seed} --time=2 \
  baseline -fe ${ft_epoch}

## infonce only global
#python run_script_0228.py ${comm_cmd} --save_dir ${save_dir} -n acdc -b ${batch_size} -pe ${pre_epoch} -fe ${ft_epoch} -s ${rand_seed} --time=3 \
...
#
#### mixup
#python run_script_0228.py ${comm_cmd} --save_dir ${save_dir} -n acdc -b ${batch_size} -pe ${pre_epoch} -fe ${ft_epoch} -s ${rand_seed} --time=4 \
#   mixup --global_features Conv5 --global_importance 1 --softenweight 1 0.1 0.01 0.001 0.0001 -g=${group_num}
#
#### multitask
python run_script_0228.py ${comm_cmd} --save_dir ${save_dir} -n acdc -b ${batch_size} -s ${rand_seed} --time=4 \
  multitask --global_features Conv5 Conv5 Conv5 --global_importance 1 0.5 0.1 --contrast_on partition patient cycle -g=${group_num} \
  -pe ${pre_epoch} -fe ${ft_epoch}
