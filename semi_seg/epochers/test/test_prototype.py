...
class TestPrototype(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._feature_importance = [1, 0.5, 0.5]
        ...
        self._optimizer = optim.Adam(chain(self._model.parameters(), self._projector.parameters()), lr=1e-3)
    ...
    def test_epocher(self):
        ...
        epocher = InfoNCEEpocher(
            ...
            unlabeled_loader=iter(self._unlabeled_loader), sup_criterion=self._sup_criterion, num_batches=100, cur_epoch=0,
            device=self._device, feature_position=self._feature_position, feature_importance=self._feature_importance
        )
        epocher.init(reg_weight=1.0, projectors_wrapper=self._projector, feature_buffers=self._buffer,
                     infoNCE_criterion=self._info_nce)
        return epocher.run()
