...
enable_acdc_all_class_train = True
...
def _run_semi(*, save_dir: str, random_seed: int = 10, num_labeled_scan: int, max_epoch: int, num_batches: int,
              ...
              rr_symmetric: str, rr_lamda: float, rr_alpha: float, align_weight: float):
    return f""" python main_da.py RandomSeed={random_seed} Trainer.name=semi \
     Trainer.save_dir={save_dir} Trainer.max_epoch={max_epoch} Trainer.num_batches={num_batches} Data.name={data_name} \
    Data.labeled_scan_num={num_labeled_scan}  Arch.checkpoint={arch_checkpoint} Optim.lr={lr:.10f} \
    ...
    CrossCorrelationParameters.hooks.align.weight={align_weight:.10f}  \
...
def run_semi_regularize(
    ...
    semi_script = [
        _run_semi(
            ...
            rr_lamda=rr_lamda, rr_alpha=rr_alpha, align_weight=align_weight
        )
...
def run_semi_regularize_with_grid_search(
        *, save_dir, random_seeds: Sequence[int] = 10, max_epoch: int, num_batches: int,
        ...
        rr_symmetric: Sequence[str], rr_lamda: Sequence[float], rr_alpha: Sequence[float], align_weight: float,
...
if __name__ == '__main__':
    ...
    save_dir = os.path.join(save_dir, f"hash_{git_hash}/{data_name}", "domain_adaptation")
