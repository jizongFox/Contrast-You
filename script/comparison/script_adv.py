...
def _run_semi(*, save_dir: str, random_seed: int = 10, num_labeled_scan: int, max_epoch: int,
    ...
    Trainer.reg_weight={adv_weight:.10f} \
    --path   config/base.yaml  config/hooks/adv.yaml  \
    """
...
def run_semi_regularize_with_grid_search(
        *, save_dir, random_seeds: Sequence[int] = 10, max_epoch: int, num_batches: int,
        data_name: str, adv_weight: Sequence[float],
        max_num: Optional[int] = 200,
) -> Iterator[List[str]]:
    param_generator = grid_search(
        random_seed=random_seeds, max_num=max_num, adv_weight=adv_weight,
    )
...
if __name__ == '__main__':
    ...
    job_generator = run_baseline_with_grid_search(
        save_dir=os.path.join(save_dir, "adv"), random_seeds=random_seeds, max_epoch=max_epoch,
    ...
    job_generator = run_semi_regularize_with_grid_search(save_dir=os.path.join(save_dir, "adv"),
                                                         random_seeds=random_seeds,
                                                         max_epoch=max_epoch, num_batches=num_batches,
                                                         data_name=data_name,
                                                         adv_weight=(0, 0.0001, 0.001, 0.01, 0.1, 1,),
                                                         )

    jobs = list(job_generator)
    logger.info(f"logging {len(jobs)} jobs")
    for job in jobs:
        submitter.submit(" && \n ".join(job), force_show=force_show, time=12, account=next(account))
