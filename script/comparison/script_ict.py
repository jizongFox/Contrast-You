...
def _run_semi(*, save_dir: str, random_seed: int = 10, num_labeled_scan: int, max_epoch: int,
    ...
    ICTMeanTeacherParameters.weight={mt_weight:.10f} \
    --path   config/base.yaml  config/hooks/ict.yaml  \
    """
...
def run_semi_regularize_with_grid_search(
        *, save_dir, random_seeds: Sequence[int] = 10, max_epoch: int, num_batches: int,
        data_name: str, mt_weight: Sequence[float],
        max_num: Optional[int] = 200,
) -> Iterator[List[str]]:
    param_generator = grid_search(
        random_seed=random_seeds, max_num=max_num, mt_weight=mt_weight
    )
...
if __name__ == '__main__':
    ...
    job_generator = run_baseline_with_grid_search(
        save_dir=os.path.join(save_dir, "ict"), random_seeds=random_seeds, max_epoch=max_epoch,
    ...
    job_generator = run_semi_regularize_with_grid_search(save_dir=os.path.join(save_dir, "ict"),
