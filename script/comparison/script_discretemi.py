...
def run_semi_regularize_with_grid_search(
        *, save_dir, random_seeds: Sequence[int] = 10, max_epoch: int, num_batches: int,
        data_name: str, con_weight: Sequence[float],
        max_num: Optional[int] = 200,
) -> Iterator[List[str]]:
    param_generator = grid_search(
        random_seed=random_seeds, max_num=max_num, con_weight=con_weight,
    )
...
if __name__ == '__main__':
    ...
    job_generator = run_baseline_with_grid_search(
        save_dir=os.path.join(save_dir, "discreteMI"), random_seeds=random_seeds, max_epoch=max_epoch,
    ...
    job_generator = run_semi_regularize_with_grid_search(save_dir=os.path.join(save_dir, "discreteMI"),
                                                         random_seeds=random_seeds,
                                                         max_epoch=max_epoch, num_batches=num_batches,
                                                         data_name=data_name,
                                                         con_weight=(0, 0.01, 0.1, 0.5, 1, 2),
                                                         )
