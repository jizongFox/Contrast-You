...
from script.script_generator_pretrain_cc import get_hyper_param_string, run_baseline_with_grid_search
...
def _run_semi(*, save_dir: str, random_seed: int = 10, num_labeled_scan: int, max_epoch: int,
              num_batches: int, arch_checkpoint: str, lr: float, data_name: str = "acdc", mx_weight: float,
              enable_bn: str):
    return f""" python main_nd.py RandomSeed={random_seed} Trainer.name=mixup \
    ...
    MixUpParams.weight={mx_weight:.10f} \
    MixUpParams.enable_bn={enable_bn} \
    --path   config/base.yaml  config/hooks/mixup.yaml  \
    """
...
def _run_semi_per_class(*, save_dir: str, random_seed: int = 10, num_labeled_scan: int, max_epoch: int,
    ...
    --path   config/base.yaml  config/hooks/mixup.yaml  \
    &&\
    python main_nd.py RandomSeed={random_seed} Trainer.name=mixup \
...
def run_semi_regularize(
    ...
    semi_script = [
        run_semi(
            ...
            mx_weight=mx_weight, enable_bn=enable_bn,
        )
...
def run_semi_regularize_with_grid_search(
        *, save_dir, random_seeds: Sequence[int] = 10, max_epoch: int, num_batches: int,
        data_name: str, mx_weight: Sequence[float], enable_bn: Sequence[str],
        max_num: Optional[int] = 200,
) -> Iterator[List[str]]:
    param_generator = grid_search(
        random_seed=random_seeds, max_num=max_num, mx_weight=mx_weight, enable_bn=enable_bn,
    )
...
if __name__ == '__main__':
    ...
    job_generator = run_baseline_with_grid_search(
        save_dir=os.path.join(save_dir, "mixup"), random_seeds=random_seeds, max_epoch=max_epoch,
    ...
    job_generator = run_semi_regularize_with_grid_search(save_dir=os.path.join(save_dir, "mixup"),
                                                         random_seeds=random_seeds,
                                                         max_epoch=max_epoch, num_batches=num_batches,
                                                         data_name=data_name,
                                                         mx_weight=(0, 0.001, 0.01, 0.1, 1),
                                                         enable_bn=("true", "false")
                                                         )
