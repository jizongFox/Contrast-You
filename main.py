...
from semi_seg.hooks.discrete_mi import MIEstimatorHook
...
with ConfigManger(base_path=os.path.join(CONFIG_PATH, "base.yaml"))(scope="base") as config:
    trainer = SemiTrainer2(model=model, labeled_loader=iter(labeled_loader),
                           unlabeled_loader=iter(unlabeled_loader), val_loader=val_loader, test_loader=test_loader,
                           sup_criterion=KL_div(), save_dir="base", max_epoch=100, num_batches=100, device="cuda",
                           configuration=config)
    # trainer.add_hook(MIEstimatorHook(name="mi_conv5", model=model, feature_name="Conv5", weight=0.1, num_clusters=20,
    #                                  num_subheads=5))
    # trainer.add_hook(MIEstimatorHook(name="mi_conv4", model=model, feature_name="Up_conv3", weight=0.1, num_clusters=20,
    #                                  num_subheads=5))
